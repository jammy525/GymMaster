		...
		<table class="mydataTable table table-striped">
			...
			foreach($data as $row)
			{
				...
				echo "<a href='".$this->Gym->createurl('GymGroup','editGroup')."/{$row['id']}' class='btn btn-flat btn-primary' title='Edit'><i class='fa fa-edit'></i></a>
					...
					echo " <a href='javascript:void(0)' data-url='".$this->request->base ."/GymAjax/viewGroupMember/{$row['id']}' class='view-grp-member btn btn-flat btn-info' id={$row['id']}><i class='fa fa-eye'></i> ".__("View")."</a>
					</td>
				</tr>
				";
			}
