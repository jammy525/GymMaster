			...
			foreach($data as $row)
			{
				...
				if($session["role_name"] == "administrator")
				{
					echo " <a href='".$this->request->base ."/GymNotice/editNotice/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
						<a href='{$this->request->base}/GymNotice/deleteNotice/{$row['id']}' class='btn btn-flat btn-danger' title='".__('Delete')."' onclick=\"return confirm('Are you sure you want to delete this product?')\"><i class='fa fa-trash'></i></a>";
				}
				echo  " <a href='javascript:void(0)' id='{$row['id']}' data-url='".$this->request->base ."/GymAjax/view_notice' class='view_notice btn btn-flat btn-info' title='".__('View')."' ><i class='fa fa-eye'></i> ".__('View')."</a>";    
				echo  "</td>";
				echo  "</tr>";
			}
			?>
