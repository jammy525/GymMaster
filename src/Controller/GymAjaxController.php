...
Class GymAjaxController extends AppController
{
	...
	public function viewAccessRole($rid)
	{
		if($this->request->is("ajax"))
		{
			$grp_tbl = TableRegistry::get("GymMember");
			$data = $grp_tbl->find("all")->where(["role_name" => $rid])->select(["first_name","last_name","image", 'email'])->hydrate(false)->toArray();
		?>
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			<h4 class="modal-title" id="gridSystemModalLabel"><?php echo __("Role Members");?></h4>
		</div>
		<div class="modal-body">
		<table class="table table-hover">
		<?php
