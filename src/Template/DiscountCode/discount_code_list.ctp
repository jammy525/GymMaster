		...
		<div class="box-body">
			...
			foreach($data as $row)
			{
				...
				echo "<td>{$row['code']}</td>
                                    <td>{$row['discount']}%</td>
                                    <td>dfhdfh</td>
                                    <td>".$GYMFunction->get_user_name($row['gym_member']['id'])."</td>
                                    <td>".date('M j, Y',strtotime($row['created_at']))." </td>
                                    <td>".date('M j, Y',strtotime($row['valid_till']))." </td>
				...
				if($session["role_name"] == "administrator" || $session["role_name"] == "franchise")
				{
					echo " <a href='".$this->request->base ."/discount-code/edit-discount-code/{$row['id']}' class='btn btn-flat btn-primary' title='".__('Edit')."'><i class='fa fa-edit'></i></a>
						<a href='{$this->request->base}/discount-code/edit-discount-code/{$row['id']}' class='btn btn-flat btn-danger' title='".__('Delete')."' onclick=\"return confirm('Are you sure you want to delete this code?')\"><i class='fa fa-trash'></i></a>";
				}
				//echo  " <a href='javascript:void(0)' id='{$row['id']}' data-url='".$this->request->base ."/GymAjax/view-discount-code' class='view_jmodal btn btn-flat btn-info' title='".__('View')."' ><i class='fa fa-eye'></i> ".__('View')."</a>";    
				echo  "</td>";
				echo  "</tr>";
			}
			?>
